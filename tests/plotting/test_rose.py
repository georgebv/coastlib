...
def test_get_rose_parameters_basic(wind_data):
    ...
    assert np.isclose(radii[0][0], 2.4416453915490024)
    assert np.isclose(radii[0][-1], 2.808966810816443)
    assert np.isclose(radii.mean(), 3.1114642491186024)
...
def test_get_rose_parameters_custom(wind_data):
    ...
    assert np.isclose(radii[0][0], 0.7606940030219351)
    assert np.isclose(radii[0][-1], 1.8099046527379772)
    assert np.isclose(radii.mean(), 1.2468840364976814)
...
def test_rose_plot_basic(wind_data):
    run_rose_plot(
        ...
        fig=None, ax=None, center_on_north=True, calm_size=1, calm_value=1, title='Wind Rose',
...
def test_rose_plot_contour(wind_data):
    run_rose_plot(
        ...
        value_bin_boundaries=np.arange(6, 18, 3), n_dir_bins=12, cmap=plt.get_cmap('jet'), rose_type='contour',
...
def test_rose_plot_contourf(wind_data):
    run_rose_plot(
        ...
        value_bin_boundaries=np.arange(6, 18, 3), n_dir_bins=12, cmap=plt.get_cmap('viridis'), rose_type='contourf',
        fig=None, ax=None, center_on_north=True, calm_size=1, calm_value=3, title='Rose Plot Contourf',
        value_name='Wind Speed', rwidths=None
    )
...
def test_rose_plot_bargeom(wind_data):
    run_rose_plot(
        ...
        value_bin_boundaries=np.arange(6, 22, 2), n_dir_bins=16, cmap=plt.get_cmap('magma'), rose_type='bar',
        fig=None, ax=None, center_on_north=False, calm_size=1, calm_value=3, title='Rose Plot Geomspace',
