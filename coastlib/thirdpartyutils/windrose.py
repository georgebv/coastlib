...
class WindroseAxes(PolarAxes):
    ...
    def from_ax(ax=None, fig=None, *args, **kwargs):
        if ax is None:
            ...
            ax = WindroseAxes(fig, rect, facecolor='w', *args, **kwargs)
            fig.add_axes(ax)
            return ax
        else:
            return ax
    ...
    def legend(self, loc='lower left', **kwargs):
        ...
        def get_labels():
            ...
            rngltb = len(labels) - 1
            labels1 = [0] * (rngltb + 1)
            for i in range(rngltb + 1):
                if i == 0:
                    labels1[i] = "< %0.1f" % (labels[i + 1])
    ...
    def _init_plot(self, direction, var, **kwargs):
        ...
        if startfromzero:
            angles = np.arange(-np.pi / nsector, -2 * np.pi + np.pi / nsector, -2 * np.pi / nsector) + np.pi / 2
        else:
...
def plot_windrose(direction_or_df, var=None, kind='contour', var_name=VAR_DEFAULT, direction_name=DIR_DEFAULT,
                  **kwargs):
    if var is None:
